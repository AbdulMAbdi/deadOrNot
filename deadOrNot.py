...
if args.ignore:
    ignoreLink = fileReader.IgnoreFile(args.ignore)
    if ignoreLink.fileComments:
        if ignoreLink.fileUrl:
            ignoreOptionFilter(args)
        else:
            processArguments(args.files, 0)
    elif ignoreLink.fileLink is not None:
        ignoreOptionFilter(args)
    else:
        print("The file contents containing the ignore link are not valid")
else:
    if args.good:
